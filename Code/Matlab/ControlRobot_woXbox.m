...
% rs = [1 + 7/16, 11 + 7/16, 2 + 7/16, 7 + 1/4, 3 + 1/8, 0, 4 + 3/4];               %Winter 2018 Limb Lengths
rs = [1 + 7/16, 4 + 1/16, 2 + 7/16, 3 + 1/4, 3 + 1/8, 0, 5 + 5/8];                 %Fall 2018 Limb Lengths
...
% Pts_Plane = [0.0842   -2.0209    2.2988;
%              11.4623   11.6209   11.7306;
%              13.6404    9.6149    9.6218];
     
Pts_Plane =    [-0.3591   -1.8378    2.1023;
                12.1268   10.2167   10.0760;
                0.2205    0.0932    0.0295];
 
% Pts_Plane =    [-0.3591   -1.8378    2.1023;
%                 12.1268   10.2167   10.0760;
%                 0.0932    0.0932    0.0295];
...
while ~button(xbox_controller, 7)                                               %While the 'back' button is not pressed...
    ...
    %Intepret input from the xbox controller and move the end effector appropriately.
    ...
    if button(xbox_controller, 3)                                               %If the x button is pressed...
        ...
        while button(xbox_controller, 3), end           %This is just a safety to make sure that matlab waits for the user to completely lift up on the "x" button before proceeding.  Otherwise Matlab can interpret a slightly held button as being multiple presses.
